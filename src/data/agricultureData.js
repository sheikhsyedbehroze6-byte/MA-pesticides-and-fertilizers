export const products = [
  {
    id: 1,
    name: "Cyclone 505 (Insecticide)",
    type: "Insecticide",
    uses: "Chlorpyrifos 50% + Cypermethrin 5% EC. Broad-spectrum control.",
    dosage: "2 ml per Litre of water.",
    diseases: ["Sucking Pests", "Caterpillars", "Bollworms"],
    benefits: "20% discount on print price.  Fast knockdown effect and long residual control.",
    composition: "Chlorpyrifos 50% + Cypermethrin 5% EC",
    image: "https://5.imimg.com/data5/FB/SH/GY/ANDROID-30875690/prod-20200616-1455468400509020022808526-jpg.jpg",
  },
  {
    id: 2,
    name: "Superstar Dodeine (Fungicide)",
    type: "Fungicide",
    uses: "Contact fungicide for controlling scab and other fungal infections.",
    dosage: "1 ml per Litre of water.",
    diseases: ["Apple Scab", "Leaf Spot", "Fruit Rot"],
    benefits: "20% discount on print price.  High-efficacy contact action with superior coverage.",
    composition: "Dodine 65% WP",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTXgiKTxt4opENrx9C-DhzrMY_6N63rkyszuw&s",
  },
  {
    id: 3,
    name: "Filpostar Proponib (Fungicide)",
    type: "Fungicide",
    uses: "Contact fungicide with protective action against broad fungal range.",
    dosage: "2.5 g per Litre of water.",
    diseases: ["Powdery Mildew", "Early Blight", "Late Blight"],
    benefits: "20% discount on print price.  Excellent rain-fastness and protective coverage.",
    composition: "Propineb 70% WP",
    image: "https://dujjhct8zer0r.cloudfront.net/media/prod_image/97ba698697ab860fb949ed2008187020-02-05-24-18-00-57.webp",
  },
  {
    id: 4,
    name: "Luna (Bayer)",
    type: "Fungicide",
    uses: "Fluopyram 17.7% + Tebuconazole 17.7%. Systemic fungicide.",
    dosage: "1 ml per Litre of water.",
    diseases: ["Powdery Mildew", "Apple Scab", "Fruit Rot"],
    benefits: "20% discount on print price.  Excellent systemic control and fruit quality.",
    composition: "Fluopyram 17.7% + Tebuconazole 17.7%",
    image: "https://cultree.in/cdn/shop/files/LunaExperience.jpg?v=1709206947",
  },
  {
    id: 5,
    name: "Antracol (Bayer)",
    type: "Fungicide",
    uses: "Propineb - Contact fungicide with broad spectrum action.",
    dosage: "2.5 g per Litre of water.",
    diseases: ["Apple Scab", "Early Blight", "Late Blight"],
    benefits: "20% discount on print price.  Strong protective action and rain-fastness.",
    composition: "Propineb 70% WP",
    image: "https://5.imimg.com/data5/SELLER/Default/2025/3/495602819/XJ/FG/VT/6399641/antracall-fungicide-500-gram.webp",
  },
  {
    id: 6,
    name: "Tropical",
    type: "Fungicide",
    uses: "Carbendazim 12% + Mancozeb 63%. Dual action systemic and contact.",
    dosage: "2 g per Litre of water.",
    diseases: ["Leaf Spot", "Blast", "Downy Mildew"],
    benefits: "20% discount on print price.  Cost-effective and highly reliable formula.",
    composition: "Carbendazim 12% + Mancozeb 63%",
    image: "https://kissanemart.com/storage/tropical-tagban-insecticide-800x800.jpg",
  },
  {
    id: 7,
    name: "Thiamethoxam",
    type: "Insecticide",
    uses: "Broad-spectrum systemic insecticide for sucking pests.",
    dosage: "0.5 g per Litre of water.",
    diseases: ["Aphids", "Whiteflies", "Jassids"],
    benefits: "20% discount on print price.  Fast action and long-lasting crop protection.",
    composition: "Thiamethoxam 25% WG",
    image: "https://5.imimg.com/data5/SELLER/Default/2023/2/IW/NT/VH/50289229/tagxone-thiamethoxam-25-wg.jpg",
  },
  {
    id: 8,
    name: "Fargo Super",
    type: "Fungicide",
    uses: "Captan 50% WP. Multi-site contact fungicide.",
    dosage: "2 g per Litre of water.",
    diseases: ["Apple Scab", "Damping Off", "Grey Mold"],
    benefits: "20% discount on print price.  Excellent resistance management and crop safety.",
    composition: "Captan 50% WP",
    image: "https://www.agricoorganics.com/wp-content/uploads/2025/02/015.jpg",
  },
  {
    id: 9,
    name: "BRAGG (Willshare)",
    type: "Herbicide",
    uses: "Paraquat Dichloride 24% SL. Non-selective contact herbicide.",
    dosage: "5 ml per Litre of water.",
    diseases: ["Annual Grasses", "Broad-leaf Weeds"],
    benefits: "20% discount on print price.  Fast acting weed control with no soil residual effect.",
    composition: "Paraquat Dichloride 24% SL",
    image: "https://appvyapar-assets.blr1.digitaloceanspaces.com/uploads/wilshire/2023-10/bigimages/b88a01ae7ce98d68af3773d4af3e3ed4.jpeg",
  },
  {
    id: 10,
    name: "Tingo (Willshare)",
    type: "Insecticide",
    uses: "Thiamethoxam + Lambda-Cyhalothrin ZC. Systemic and contact action.",
    dosage: "0.4 ml per Litre of water.",
    diseases: ["Fruit Borer", "Shoot Borer", "Sucking Pests"],
    benefits: "20% discount on print price.  Advanced ZC formulation for superior performance and stability.",
    composition: "Thiamethoxam + Lambda-Cyhalothrin ZC",
    image: "/tingo.webp",
  },
  {
    id: 11,
    name: "Difenoconazole 25% (Willowood)",
    type: "Fungicide",
    uses: "Systemic fungicide.",
    dosage: "0.5 ml per Litre of water.",
    diseases: ["Apple Scab", "Leaf Spot", "Rust"],
    benefits: "20% discount on print price.  Long-lasting protection and fast systemic movement.",
    composition: "Difenoconazole 25% EC",
    image: "https://5.imimg.com/data5/SELLER/Default/2024/10/459139118/QZ/EE/KR/131299105/94image-wilcore-500x500.jpg",
  },
  {
    id: 12,
    name: "Willowood Carmel (Fungicide)",
    type: "Fungicide",
    uses: "Carbendazim 12% + Mancozeb 63% WP. Dual action (systemic and contact) for broad-spectrum fungal control.",
    dosage: "2 g per Litre of water.",
    diseases: ["Apple Scab", "Powdery Mildew", "Early/Late Blight", "Rust"],
    benefits: "20% discount on print price. Dual action (preventive and curative) with long-lasting residual effect.",
    composition: "Carbendazim 12% + Mancozeb 63% WP",
    image: "https://highyieldsagro.com/wp-content/uploads/2025/02/Willowood-Carmel-large.webp",
  },
  {
    id: 13,
    name: "Kozen (Insecticide)",
    type: "Insecticide",
    uses: "Advanced systemic insecticide for controlling lepidopteran pests.",
    dosage: "0.4 ml per Litre of water.",
    diseases: ["Fruit Borer", "Caterpillars", "Leaf Miner"],
    benefits: "20% discount on print price.  Excellent crop safety and long-lasting efficacy.",
    composition: "Chlorantraniliprole 18.5% SC",
    image: "https://cdn.shopify.com/s/files/1/0722/2059/files/Reserved_ImageAttachment__13___ProductImage1_32___0d3ef08ce6a7403ea5e9dcb7fa45a078_1___1.png?v=1752152468",
  },
  {
    id: 14,
    name: "Gulftop (Herbicide)",
    type: "Herbicide",
    uses: "Selective post-emergence herbicide for broad-leaf weed control.",
    dosage: "1 ml per Litre of water.",
    diseases: ["Broad-leaf Weeds", "Annual Grasses"],
    benefits: "20% discount on print price.  Safe for the main crop while destroying competing weeds.",
    composition: "Glyphosate 41% SL",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6nasU2IU8ZEro3M-oaMI5KeHcoEA4F-nzWw&s",
  },
  {
    id: 15,
    name: "Heinekey (Humic Acid)",
    type: "Plant Tonic",
    uses: "High-grade Humic acid for soil enrichment and root stimulation.",
    dosage: "5 ml per Litre of water (Foliar) / 1kg per acre (Soil).",
    diseases: ["Nutrient Deficiency", "Poor Root Growth"],
    benefits: "20% discount on print price.  Improves soil structure and nutrient uptake efficiency.",
    composition: "Humic Acid 12% + Fulvic Acid",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTHlgpnfoJw7BGTKaiRC1XYl_rlCAxrOxKL2w&s",
  },
  {
    id: 16,
    name: "IPL 5G Neo+",
    type: "Bio-Stimulant",
    uses: "Advanced plant growth promoter and yield enhancer.",
    dosage: "2 ml per Litre of water.",
    diseases: ["Stunted Growth", "high Yield"],
    benefits: "20% discount on print price.  Boosts plant immunity and improves fruit size/quality.",
    composition: "Multi-micronutrients & Bio-stimulants",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR3arIuRxI754NrO4GwFUuvOPxCpHqnaGaqcA&s",
  },
  {
    id: 17,
    name: "Wilogore (Insecticide)",
    type: "Insecticide",
    uses: "Powerful contact and systemic insecticide.",
    dosage: "1.5 ml per Litre of water.",
    diseases: ["Sucking Pests", "Aphids", "Thrips"],
    benefits: "20% discount on print price.  Provides rapid knockdown and broad-spectrum control.",
    composition: "Chlorpyrifos 20% EC",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQv34t2ZrFcv3iPyT27lNOJVXVjeC59a96ZeQ&s",
  },
  {
    id: 18,
    name: "IPL Sanjeevni (Trichoderma Viride)",
    type: "Bio-Fungicide",
    uses: "Biological fungicide for soil-borne disease management.",
    dosage: "5-10 g per Litre of water.",
    diseases: ["Root Rot", "Wilt", "Damping Off"],
    benefits: "20% discount on print price.  Eco-friendly biological control with no chemical residues.",
    composition: "Trichoderma Viride",
    image: "https://www.farmersstop.com/cdn/shop/files/35963201716307_360x480.png?v=1748675031",
  },
  {
    id: 19,
    name: "IPL Vamshakti (VAM)",
    type: "Bio-Fertilizer",
    uses: "Vesicular Arbuscular Mycorrhiza for phosphate mobilization.",
    dosage: "4 kg per acre.",
    diseases: ["Phosphorus Deficiency", "Soil Exhaustion"],
    benefits: "20% discount on print price.  Significantly increases root surface area and drought tolerance.",
    composition: "Vesicular Arbuscular Mycorrhiza (VAM)",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRzDdlD5jFnmvQlnkPxcwjXjiP8af0K1zYESg&s",
  },
  {
    id: 20,
    name: "IPL Neemkavach (Neem Oil)",
    type: "Bio-Insecticide",
    uses: "Cold-pressed Neem oil based botanical insecticide.",
    dosage: "3-5 ml per Litre of water.",
    diseases: ["Whiteflies", "Mites", "Leaf Hopper"],
    benefits: "20% discount on print price.  Natural pest repellent and anti-feedant properties.",
    composition: "Neem Oil (Azadirachtin)",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQFPhZWCKsznPzuVNlrlwqU0uxLFWOcjNmSTA&s",
  },
  {
    id: 21,
    name: "A-MIX (Herbicide)",
    type: "Herbicide",
    uses: "Mixed-action herbicide for comprehensive weed management.",
    dosage: "2 ml per Litre of water.",
    diseases: ["Mixed Weed Population"],
    benefits: "20% discount on print price.  Effective control of tough-to-kill weeds in orchards.",
    composition: "2,4-D Ethyl Ester 38% EC",
    image: "https://5.imimg.com/data5/SELLER/Default/2024/10/458420997/YG/WV/CE/102230489/green-mix-herbicide-500x500.jpg",
  },
  {
    id: 22,
    name: "Saffire Bublin (Liquid Fertilizer)",
    type: "Fertilizer",
    uses: "Premium liquid fertilizer for quick nutrient absorption and leaf greening.",
    dosage: "3 ml per Litre of water.",
    diseases: ["Yellowing Leaves", "Nutrient Stress"],
    benefits: "20% discount on print price.  Fast-acting liquid formula for instant results.",
    composition: "NPK + Micronutrients (Liquid)",
    image: "https://5.imimg.com/data5/NSDMERP/Default/2026/1/575333124/EH/ID/FV/68915898/68915898-product-1768220776639-250x250.png",
  },
  {
    id: 23,
    name: "Silixol (Fertilizer)",
    type: "Fertilizer",
    uses: "Silicon-based specialty fertilizer for plant strength and stress tolerance.",
    dosage: "1-2 ml per Litre of water.",
    diseases: ["Environmental Stress", "Weak Stems"],
    benefits: "20% discount on print price.  Enhances structural strength and resistance to pests.",
    composition: "Ortho Silicic Acid (OSA)",
    image: "https://cdn.shopify.com/s/files/1/0722/2059/files/silixol-file-61.jpg?v=1737482833",
  },
  {
    id: 24,
    name: "Clear Cut (Systemic Herbicide)",
    type: "Herbicide",
    uses: "Broad-spectrum systemic herbicide for non-selective weed control.",
    dosage: "5-10 ml per Litre of water.",
    diseases: ["Annual Weeds", "Perennial Grasses"],
    benefits: "20% discount on print price.  Complete root-to-leaf kill for long-lasting weed control.",
    composition: "Glyphosate 41% SL",
    image: "https://images.jdmagicbox.com/quickquotes/images_main/super-clear-cut-372666588-z38e8.jpg",
  },
  {
    id: 25,
    name: "Chaubatia Tree Paste",
    type: "Fungicidal Wound Paste",
    uses: "Protective paste for sealing pruning cuts and treating tree wounds.",
    dosage: "Apply directly to cut surfaces or wounds.",
    diseases: ["Canker", "Fungal Infections", "Wound Rot"],
    benefits: "20% discount on print price.  Forms a durable protective barrier against weather and pests.",
    composition: "Copper Carbonate + Red Lead + Lanoline",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR3dvS4vYRGfvxbCqxXWjs7eK9lmSLiZFrOuQ&s",
  },
  {
    id: 26,
    name: "Merrito (Fungicide)",
    type: "Fungicide",
    uses: "High-performance fungicide for fruit and vegetable crops.",
    dosage: "1-2 g per Litre of water.",
    diseases: ["Powdery Mildew", "Leaf Spot"],
    benefits: "20% discount on print price.  Excellent preventative and curative action.",
    composition: "Myclobutanil 10% WP",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQT4ukTV7KHWsg8IwZnrYa51A8AUABauc1d6A&s",
  },
  {
    id: 27,
    name: "Captaf (Fungicide)",
    type: "Fungicide",
    uses: "Broad-spectrum contact fungicide for various fruit crops.",
    dosage: "2 g per Litre of water.",
    diseases: ["Apple Scab", "Leaf Spot", "Damping Off"],
    benefits: "20% discount on print price.  Reliable protection and resistance management tool.",
    composition: "Captan 50% WP",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQJ1zgh6MbbsaEgGkTHUNybCylA-Ghvl6a-tg&s",
  },
  {
    id: 28,
    name: "Green Valley (Calcium Liquid)",
    type: "Liquid Fertilizer",
    uses: "Soluble calcium source for preventing nutrient deficiencies.",
    dosage: "2-3 ml per Litre of water.",
    diseases: ["Bitter Pit", "Fruit Cracking", "Tip Burn"],
    benefits: "20% discount on print price.  Rapidly absorbed for better fruit quality and shelf life.",
    composition: "Calcium Nitrate (Liquid)",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT50BGzJVICHC5w1QriAz5h7JfHYFExCtdfhw&s",
  },
  {
    id: 29,
    name: "Filbardy (Plant Protein Liquid)",
    type: "Bio-Stimulant",
    uses: "Protein-based liquid supplement for plant vigor and stress recovery.",
    dosage: "2 ml per Litre of water.",
    diseases: ["Stress Recovery", "Poor Fruit Set"],
    benefits: "20% discount on print price.  Natural growth booster with essential amino acids.",
    composition: "Amino Acids + Peptides",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRRYv8O3vHRDVcJcklLz-TxMc2P3BLePzrJpg&s",
  },
  {
    id: 30,
    name: "Bordeaux (Latex Paste)",
    type: "Fungicidal Paste",
    uses: "Traditional latex-based copper paste for trunk protection.",
    dosage: "Paint on trunk base or large pruning wounds.",
    diseases: ["Collar Rot", "Gummosis", "Canker"],
    benefits: "20% discount on print price.  Long-lasting adhesion and excellent disease suppression.",
    composition: "Copper Sulphate + Lime",
    image: "https://cpimg.tistatic.com/8361085/s/4/devamrut-bordeaux.jpg",
  },
  {
    id: 31,
    name: "Matrix (Systemic Fungicide)",
    type: "Fungicide",
    uses: "Advanced systemic fungicide with dual action for internal protection.",
    dosage: "1 ml per Litre of water.",
    diseases: ["Blast", "Rust", "Sheath Blight"],
    benefits: "20% discount on print price.  Fast movement within the plant for complete coverage.",
    composition: "Tricyclazole 75% WP",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTVdoDLPFDovM1bl72AyoMO904EVH2Wm3_sYg&s",
  },
  {
    id: 32,
    name: "Rhizo (Biological Fertilizer)",
    type: "Bio-Fertilizer",
    uses: "Rhizobium based biological fertilizer for nitrogen fixation.",
    dosage: "10 ml per Litre for seed treatment or soil drench.",
    diseases: ["Nitrogen Deficiency", "Poor Soil Fertility"],
    benefits: "20% discount on print price.  Enhances soil health and reduces dependence on urea.",
    composition: "Rhizobium Sp.",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcThpYHnlmTTa3AS8P-TvTmZrF3egUAOVdIK8w&s",
  },
  {
    id: 33,
    name: "Nischit (Systemic Fungicide)",
    type: "Fungicide",
    uses: "Advanced systemic fungicide for effective disease control and prevention.",
    dosage: "1.5-2 g per Litre of water.",
    diseases: ["Apple Scab", "Leaf Spot", "Powdery Mildew"],
    benefits: "20% discount on print price.  Fast systemic action for internal plant protection.",
    composition: "Carbendazim 50% WP",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTJI9khL3IwXLtk2Tgi-27WvBo0unyVTFVySw&s",
  },
  {
    id: 34,
    name: "Evoke (Herbicide)",
    type: "Herbicide",
    uses: "Selective herbicide for the control of tough weeds in agricultural fields.",
    dosage: "1-1.5 ml per Litre of water.",
    diseases: ["Broad-leaf Weeds", "Sedges"],
    benefits: "20% discount on print price.  Targeted weed control with excellent crop safety.",
    composition: "Ethoxysulfuron 15% WDG",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQik3DWt_CaHEbaNRbjatTua6NQ4A9dAscr2w&s",
  },
  {
    id: 35,
    name: "Fuzila (Fungicide)",
    type: "Fungicide",
    uses: "Broad-spectrum fungicide for various fruits and vegetables.",
    dosage: "2 g per Litre of water.",
    diseases: ["Early Blight", "Late Blight", "Fruit Rot"],
    benefits: "20% discount on print price.  Reliable protection against a wide range of fungal pathogens.",
    composition: "Flusilazole 40% EC",
    image: "https://5.imimg.com/data5/SELLER/Default/2022/6/IL/SW/AF/119611945/filia-se-1--500x500.jpg",
  },
  {
    id: 36,
    name: "Gib-Gib (Plant Growth Regulator)",
    type: "Growth Regulator",
    uses: "Gibberellic acid based formulation for plant growth stimulation.",
    dosage: "0.5-1 g per Litre of water.",
    diseases: ["Poor Fruit Size", "Stunted Growth"],
    benefits: "20% discount on print price.  Enhances cell elongation and improves fruit quality.",
    composition: "Gibberellic Acid 0.001%",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTTwcx9b7mD0ck83cIUYi4tLRI32JXPF7_9ZA&s",
  },
  {
    id: 37,
    name: "Filquart (Contact Herbicide)",
    type: "Herbicide",
    uses: "Fast-acting contact herbicide for quick weed knockdown.",
    dosage: "5-7 ml per Litre of water.",
    diseases: ["Annual Grasses", "Broad-leaf Weeds"],
    benefits: "20% discount on print price.  Rapid action with visible results within hours.",
    composition: "Paraquat Dichloride 24% SL",
    image: "https://5.imimg.com/data5/SELLER/Default/2025/3/498386206/IY/BQ/RH/77561523/1-litre-wilquat-contact-agricultural-herbicides.jpg",
  },
  {
    id: 38,
    name: "Rubigan D (Systemic Fungicide)",
    type: "Fungicide",
    uses: "Highly effective systemic fungicide specifically for scab and mildew control.",
    dosage: "0.4 ml per Litre of water.",
    diseases: ["Apple Scab", "Powdery Mildew"],
    benefits: "20% discount on print price.  Superior systemic movement for comprehensive protection.",
    composition: "Fenarimol 12% EC",
    image: "https://5.imimg.com/data5/JN/GD/TF/SELLER-23513554/rubigan-d-fungicide-500x500.jpg",
  },
  {
    id: 39,
    name: "NPK (Water Soluble Fertilizer)",
    type: "Fertilizer",
    uses: "Balanced NPK fertilizer for overall plant nutrition and growth.",
    dosage: "5-10 g per Litre of water.",
    diseases: ["Nutrient Deficiency", "Slow Growth"],
    benefits: "20% discount on print price.  100% water-soluble for quick nutrient availability.",
    composition: "NPK 19:19:19",
    image: "https://m.media-amazon.com/images/I/61UUR5NjhfL._AC_UF1000,1000_QL80_.jpg",
  },
  {
    id: 40,
    name: "Triofil (Fungicide)",
    type: "Fungicide",
    uses: "Triazole based systemic fungicide for cereal and fruit crops.",
    dosage: "1 ml per Litre of water.",
    diseases: ["Rust", "Smut", "Scab"],
    benefits: "20% discount on print price.  Protective and curative action with long-lasting effect.",
    composition: "Propiconazole 25% EC",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSHfNmJO0S2qtjRlntd8ZiOkEQOQ_N0CwRwew&s",
  },
  {
    id: 41,
    name: "Rootolar (Bio Fertilizer)",
    type: "Bio-Fertilizer",
    uses: "Microbial bio-fertilizer for enhanced root health and soil fertility.",
    dosage: "10 ml per Litre of water.",
    diseases: ["Poor Root Development", "Soil Exhaustion"],
    benefits: "20% discount on print price.  Natural way to improve soil biology and plant vigor.",
    composition: "Bacillus Subtilis",
    image: "https://5.imimg.com/data5/GLADMIN/Default/2022/12/WS/VI/XC/35854812/new-product-250x250.jpeg",
  },
  {
    id: 42,
    name: "Life80 (Spreader, Non-ionic Surfactant)",
    type: "Adjuvant",
    uses: "Premium quality spreader and activator for better spray efficacy.",
    dosage: "0.5 ml per Litre of spray solution.",
    diseases: ["Poor Spray Coverage", "Run-off"],
    benefits: "20% discount on print price.  Improves coverage, penetration, and rain-fastness of sprays.",
    composition: "Alkyl Aryl Polyalkoxylate",
    image: "https://5.imimg.com/data5/PJ/FU/HE/SELLER-26762835/amino-acid-powder-80-agricultural-fertilizer-500x500.jpg",
  },
  {
    id: 43,
    name: "Fxapp (Plant Growth Regulator)",
    type: "Growth Regulator",
    uses: "Specialized PGR for balancing vegetative and reproductive growth.",
    dosage: "1 ml per Litre of water.",
    diseases: ["Excessive Vegetative Growth", "Poor Flowering"],
    benefits: "20% discount on print price.  Promotes better flowering and fruit set.",
    composition: "Paclobutrazol 23% SC",
    image: "https://krishimart.in/public/uploads/all/sSoDaErVuZ3gM3yVzJFcDWeuuBUasekV4kORuUIj.jpg",
  },
  {
    id: 44,
    name: "Adunik Starcare (Insecticide)",
    type: "Insecticide",
    uses: "Modern insecticide for controlling a wide spectrum of sucking and chewing pests.",
    dosage: "1 ml per Litre of water.",
    diseases: ["Aphids", "Thrips", "Caterpillars"],
    benefits: "20% discount on print price.  Advanced formulation for superior pest control.",
    composition: "Imidacloprid 17.8% SL",
    image: "https://adhunikcropcare.com/uploads/product/22.jpg",
  },
  {
    id: 45,
    name: "Mitofix (Insecticide)",
    type: "Insecticide",
    uses: "Effective insecticide and acaricide for controlling all stages of mites.",
    dosage: "2 ml per Litre of water.",
    diseases: ["Red Mites", "Spider Mites", "Yellow Mites"],
    benefits: "20% discount on print price. Excellent knock-down effect and long residual control.",
    composition: "Propargite 57% EC",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRvWe35Py5mJbc-yC870sW84RKBnmh5ipPW9w&s",
  },
  {
    id: 46,
    name: "Bublin (NPK 11:11:8 Suspension Fertilizer)",
    type: "Fertilizer",
    uses: "High-quality suspension fertilizer enriched with Zinc and Boron for balanced nutrition.",
    dosage: "2 ml per Litre of water.",
    diseases: ["Nutrient Deficiency", "Poor Flower Set", "Fruit Drop"],
    benefits: "20% discount on print price. Enhances flower retention, fruit setting, and overall crop yield.",
    composition: "NPK 11:11:8 + Zinc + Boron Suspension",
    image: "https://5.imimg.com/data5/NSDMERP/Default/2026/1/575333124/EH/ID/FV/68915898/68915898-product-1768220776639-250x250.png",
  },
  {
    id: 47,
    name: "Tata Takumi (Insecticide)",
    type: "Insecticide",
    uses: "Advanced systemic insecticide for controlling caterpillars and borers in various crops.",
    dosage: "0.5 g per Litre of water.",
    diseases: ["Stem Borer", "Leaf Folder", "Fruit Borer", "Diamondback Moth"],
    benefits: "20% discount on print price. Fast knockdown effect, stops pest feeding instantly, and provides long-lasting control.",
    composition: "Flubendiamide 20% WG",
    image: "https://kavungal.in/wp-content/uploads/2024/07/IMG-20240730-WA0014.jpg",
  },
  {
    id: 48,
    name: "Sikri Vermicompost (Organic Fertilizer)",
    type: "Bio-Fertilizer",
    uses: "High-quality nutrient-rich soil amendment created through controlled earthworm decomposition.",
    dosage: "2 kg – 4 kg per tree or 250g per potted plant.",
    diseases: ["Soil Exhaustion", "Poor Soil Structure", "Nutrient Deficiency"],
    benefits: "20% discount on print price. Enriches soil with macronutrients, improves aeration, and boosts microbial activity.",
    composition: "Organic Carbon, Nitrogen, Phosphorus, Potassium, and Micronutrients",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQq2RlXn0pS7Jz4q44QSfjiLszRJXv2xMiamQ&s",
  },
  {
    id: 49,
    name: "All Clear (Herbicide)",
    type: "Herbicide",
    uses: "Paraquat Dichloride 24% SL. Non-selective contact herbicide for broad-spectrum weed control.",
    dosage: "6-7 ml per Litre of water.",
    diseases: ["Broadleaf Weeds", "Grasses", "Aquatic Weeds"],
    benefits: "20% discount on print price. Fast-acting contact action, becomes inactive on contact with soil.",
    composition: "Paraquat Dichloride 24% SL",
    image: "https://dujjhct8zer0r.cloudfront.net/media/prod_image/2200404361746272971.webp",
  },
  {
    id: 50,
    name: "Novathion (Insecticide)",
    type: "Insecticide",
    uses: "Ethion 50% EC. Organophosphorus insecticide for controlling mites and sucking pests.",
    dosage: "2 ml per Litre of water.",
    diseases: ["Mites", "Thrips", "Aphids", "Caterpillars"],
    benefits: "20% discount on print price. Excellent ovicidal action and effective against resistant mite populations.",
    composition: "Ethion 50% EC",
    image: "https://tropicalagro.in/wp-content/uploads/2024/05/NOVATHION.png",
  },
  {
    id: 51,
    name: "Governor (Systemic Insecticide)",
    type: "Insecticide",
    uses: "Advanced systemic formulation for internal protection against stubborn pests.",
    dosage: "1-1.5 ml per Litre of water.",
    diseases: ["Stem Borer", "Leaf Miner", "Potato Beetle"],
    benefits: "20% discount on print price. Quick absorption and long-lasting internal protection for the plant.",
    composition: "Systemic Insecticide Formulation",
    image: "https://www.filindustries.com/images/products/governor.jpg",
  },
  {
    id: 52,
    name: "Proma Gro (Chaubatia Paste)",
    type: "Fungicidal Wound Dressing",
    uses: "Specialized wound-dressing fungicide for sealing pruning cuts and treating tree wounds.",
    dosage: "Apply directly with a brush to cut surfaces or wounds.",
    diseases: ["Canker", "Wound Rot", "Pruning Infections"],
    benefits: "20% discount on print price. Forms a weather-resistant protective barrier that accelerates healing.",
    composition: "Copper Carbonate + Red Lead + Specialized Dressing Oil",
    image: "https://tivishaenterprises.in/wp-content/uploads/2024/03/Screenshot-2024-03-12-at-1.49.01-PM-300x300.png.webp",
  },

  {
    id: 53,
    name: "Syngenta Alika (Insecticide)",
    type: "Insecticide",
    uses: "Thiamethoxam 12.6% + Lambda-cyhalothrin 9.5% ZC. Advanced dual-action systemic and contact insecticide for sucking & chewing pests.",
    dosage: "0.5 ml per Litre of water (100 ml per 200 Litres).",
    diseases: ["Codling Moth", "Aphids", "Jassids", "Thrips", "Fruit Borer", "Caterpillars"],
    benefits: "20% discount on print price. Instant knockdown plus long-lasting systemic protection against codling moth and sucking pests.",
    composition: "Thiamethoxam 12.6% + Lambda-cyhalothrin 9.5% ZC",
    image: "/alika.png",
  },
  {
    id: 54,
    name: "Tata Sarthak (Fungicide)",
    type: "Fungicide",
    uses: "Hexaconazole 5% EC. High-efficiency systemic fungicide for broad disease protection in fruit crops.",
    dosage: "1 ml per Litre of water.",
    diseases: ["Apple Scab", "Powdery Mildew", "Leaf Spot"],
    benefits: "20% discount on print price. High systemic protection from Tata Rallis for scab and powdery mildew control.",
    composition: "Hexaconazole 5% EC (Tata Product)",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSeKVrFyGuRUpUosMxaABkS-G3Y6neen24tMZ53rBviy5ZIMpl9mvd-QcM&s=10",
  },
  {
    id: 55,
    name: "Dawn 7000 (Insecticide)",
    type: "Insecticide",
    uses: "High-efficacy insecticide formulation engineered for rapid knockdown of fruit borers and sucking pests.",
    dosage: "1.5 ml per Litre of water.",
    diseases: ["Aphids", "Thrips", "Caterpillars", "Fruit Borer"],
    benefits: "20% discount on print price. Fast knockdown action and long-lasting crop protection.",
    composition: "Broad-Spectrum Insecticide Formulation",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSIfg-fjsk18uYcLjtHgQhqbBw5HZs17V7PZQVPNTsmnw&s=10",
  },
  {
    id: 56,
    name: "Jabasdas (Herbicide)",
    type: "Herbicide",
    uses: "Post-emergence selective herbicide for fast and effective weed clearance in orchards and fields.",
    dosage: "2.5 ml per Litre of water.",
    diseases: ["Broad-leaf Weeds", "Annual Grasses", "Orchard Weeds"],
    benefits: "20% discount on print price. Rapid action on broadleaf and grassy weeds with zero soil toxicity.",
    composition: "Post-Emergence Herbicide Formulation",
    image: "https://www.napanta.com/app-img/product/ipl_gajab.png",
  },
  {
    id: 57,
    name: "Stargem-45 (Fungicide)",
    type: "Fungicide",
    uses: "Mancozeb 75% WP. Multi-site contact fungicide with broad-spectrum protective action against blight and scab.",
    dosage: "2.5 g per Litre of water.",
    diseases: ["Apple Scab", "Early Blight", "Late Blight", "Downy Mildew"],
    benefits: "20% discount on print price. Reliable multi-site contact action preventing fungicide resistance.",
    composition: "Mancozeb 75% WP",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSPetVmezeFNMDZk6LTUtdkaWCKASohZIEW4IwGDOHVDw&s=10",
  },
  {
    id: 58,
    name: "Tegata (Insecticide)",
    type: "Insecticide",
    uses: "Potent contact & systemic insecticide specially formulated for controlling spider mites, thrips, and sucking insects.",
    dosage: "1 ml per Litre of water.",
    diseases: ["Spider Mites", "Thrips", "Whiteflies", "Leaf Hopper"],
    benefits: "20% discount on print price. Excellent translaminar penetration and long residual protection.",
    composition: "Contact & Systemic Insecticide Formulation",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQM8SVZBiX8PNhFXjsLsjAqpbRc38179heMyvFrsTCHKg&s=10",
  },
  {
    id: 60,
    name: "Savera Systemic Insecticide",
    type: "Insecticide",
    uses: "Advanced systemic insecticide for internal plant protection against sucking pests, caterpillars, and borers.",
    dosage: "1 ml per Litre of water.",
    diseases: ["Aphids", "Thrips", "Whiteflies", "Caterpillars"],
    benefits: "20% discount on print price. Deep vascular absorption providing extended immunity against pest damage.",
    composition: "Systemic Insecticide Formulation",
    image: "https://5.imimg.com/data5/SELLER/Default/2025/10/550505320/QX/OR/GJ/109535619/whatsapp-image-2025-10-03-at-11-11-08-am.jpeg",
  },
];


export const diseases = [
  {
    id: "d1",
    crop: "Apple / سیب",
    name: "Apple Scab",
    nameUrdu: "ایپل سکیب (سیب کی خارش / فنگس)",
    symptoms: "Olive-green velvet spots on leaves, dark corky scabs on fruit causing deformation.",
    symptomsUrdu: "پتوں پر زیتونی سبز دھبے، پھل پر سیاہ مخملی اور کھردری نشانات۔",
    cure: "Superstar Dodine 65% WP or Bayer Antracol (Propineb 70% WP)",
    cureUrdu: "سپر سٹار ڈوڈائن 65% ڈبلیو پی یا بائر اینٹراکول (پروپینیب)",
    dosage: "1g Dodine or 2.5g Antracol per Litre of water. Spray every 10-14 days.",
    dosageUrdu: "1 گرام ڈوڈائن یا 2.5 گرام اینٹراکول فی لیٹر پانی۔ ہر 10-14 دن بعد۔",
    severity: "High",
    image: "/apple-scab.webp"
  },
  {
    id: "d2",
    crop: "Apple / سیب",
    name: "San Jose Scale",
    nameUrdu: "سان جوز سکیل (چھال کا کیڑا)",
    symptoms: "Ashy-grey crusty patches on bark, small red halos/spots on developing apples.",
    symptomsUrdu: "چھال پر سرمئی سخت تہہ، سیب کے پھل پر لال نقطے یا نشانات۔",
    cure: "Horticultural Mineral Oil (HMO) + Cyclone 505 (Chlorpyrifos 50% + Cypermethrin 5%)",
    cureUrdu: "ہارٹیکلچرل منرل آئل + سائکلون 505 (Chlorpyrifos + Cypermethrin)",
    dosage: "20ml HMO + 1.5ml Cyclone 505 per Litre of water at Green Tip stage.",
    dosageUrdu: "20 ملی لیٹر منرل آئل + 1.5 ملی لیٹر دوا فی لیٹر (گرین ٹپ پر)۔",
    severity: "High",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQIt4InCwngKvAjcBBYUGHAC4Z6kQ-k7ORBRw&s"
  },
  {
    id: "d3",
    crop: "Apple / سیب",
    name: "Powdery Mildew",
    nameUrdu: "پاؤڈری پھپھوندی (سفید پاؤڈر)",
    symptoms: "White, powdery fungal coating on young leaves, shoots, and flower buds.",
    symptomsUrdu: "نئے پتوں، ٹہنیوں اور پھول کی کلیوں پر سفید پاؤڈر جیسی پھپھوندی۔",
    cure: "Luna Experience (Fluopyram + Tebuconazole) or Sulphur 80% WDG",
    cureUrdu: "لونا ایکسپیرینس (Bayer Luna) یا سلفر 80% ڈبلیو ڈی جی",
    dosage: "1 ml Luna or 3 g Sulphur per Litre of water at Pink Bud stage.",
    dosageUrdu: "1 ملی لیٹر لونا یا 3 گرام سلفر فی لیٹر پانی (پنک بڈ سٹیج پر)۔",
    severity: "Medium",
    image: "https://www.gardendesign.com/pictures/images/400x320Exact_0x0/site_3/powdery-mildew-powdery-mildew-on-squash-leaf-shutterstock-com_13365.jpg"
  },
  {
    id: "d4",
    crop: "Apple / سیب",
    name: "European Red Mite",
    nameUrdu: "سرخ مکڑی (یورپی ریڈ مائٹ)",
    symptoms: "Leaves turn dull bronze or rusty brown, severe defoliation, reduced fruit size.",
    symptomsUrdu: "پتے بھورے یا پیتل جیسے ہو جاتے ہیں، پتوں کا جھڑنا اور پھل چھوٹا رہنا۔",
    cure: "Mitofix (Propargite 57% EC) or Spiromesifen 22.9% SC",
    cureUrdu: "مائٹو فکس (پروپارگائٹ 57%) یا سپرومیسیفین",
    dosage: "1.5 ml Mitofix per Litre of water when mite population exceeds 5 per leaf.",
    dosageUrdu: "1.5 ملی لیٹر مائٹو فکس فی لیٹر پانی۔",
    severity: "Medium",
    image: "https://content.ces.ncsu.edu/media/images/Transparency0235.jpg"
  },
  {
    id: "d5",
    crop: "Apple / سیب",
    name: "Woolly Aphid",
    nameUrdu: "اُونی مکھو (سفید روئی دار کیڑا)",
    symptoms: "White cottony masses covering bark crevices, twigs, and root collars.",
    symptomsUrdu: "شاخوں، چھال کی دراڑوں اور جڑوں پر سفید روئی نما کیڑے۔",
    cure: "Cyclone 505 (Chlorpyrifos 50% + Cypermethrin 5% EC) or Thiamethoxam 25% WG",
    cureUrdu: "سائکلون 505 (Chlorpyrifos + Cypermethrin) یا تھیامیتھوکسام",
    dosage: "2 ml Cyclone 505 per Litre of water thoroughly covering bark crevices.",
    dosageUrdu: "2 ملی لیٹر سائکلون 505 فی لیٹر پانی (چھال پر اچھی طرح سپرے کریں)۔",
    severity: "High",
    image: "https://www.cropscience.bayer.co.nz/-/media/bcs-inter/ws_newzealand/pests/insects/woolly-apple-aphid/waa-shoots-page.jpg?h=696&w=900&la=en&hash=FFF8335E2DAD650AF25E1A098DF3E9CC"
  },
  {
    id: "d6",
    crop: "Apple / سیب",
    name: "Collar Rot / Root Rot",
    nameUrdu: "کالر راٹ (تنے اور جڑ کا سڑنا)",
    symptoms: "Dark, water-soaked brown decay of bark at soil line, wilting, tree decline.",
    symptomsUrdu: "مٹی کے قریب تنے کی چھال کا سیاہ ہو کر سڑنا اور درخت کا سوکھنا۔",
    cure: "Ridomil Gold (Metalaxyl 8% + Mancozeb 64% WP) or Copper Oxychloride 50% WP",
    cureUrdu: "ریڈومل گولڈ (Metalaxyl + Mancozeb) یا کاپر آکسی کلورائیڈ",
    dosage: "3 g per Litre water. Drench 10-15 Litres solution around tree trunk base.",
    dosageUrdu: "3 گرام فی لیٹر پانی (تنے اور جڑوں کے ارد گرد 10-15 لیٹر محلول ڈالیں)۔",
    severity: "High",
    image: "https://cdn.shopify.com/s/files/1/1254/5779/files/Collar-Rot2.jpg?v=1619632720"
  },
  {
    id: "d7",
    crop: "Apple / سیب",
    name: "Alternaria Leaf Blight",
    nameUrdu: "الٹرنیریا بلائٹ (پتوں پر گہرے دھبے)",
    symptoms: "Circular dark brown spots on leaves surrounded by purple halos, early leaf fall.",
    symptomsUrdu: "پتوں پر گہرے بھورے گول دھبے جن کے گرد جامنی دائرہ ہو، پتے جھڑنا۔",
    cure: "Difenoconazole 25% EC (Score / Willowood) or Mancozeb 75% WP",
    cureUrdu: "ڈائفینوکونازول 25% ای سی (Willowood) یا مینکوزیب",
    dosage: "0.5 ml Difenoconazole or 2.5g Mancozeb per Litre of water.",
    dosageUrdu: "0.5 ملی لیٹر ڈائفینوکونازول یا 2.5 گرام مینکوزیب فی لیٹر پانی۔",
    severity: "Medium",
    image: "https://cdn.shopify.com/s/files/1/0762/3639/0696/files/268e8bc6-04ba-4520-9c79-3d7cfadd8fe4_11zon.webp?v=1739606163"
  },
  {
    id: "d8",
    crop: "Apple / سیب",
    name: "Fire Blight",
    nameUrdu: "فائر بلائٹ (جلی ہوئی شاخیں)",
    symptoms: "Blossoms, shoots, and leaves suddenly turn black and shrivel as if scorched by fire.",
    symptomsUrdu: "پھول، ٹہنیاں اور پتے اچانک سیاہ ہو کر ایسے مرجھاتے ہیں جیسے آگ سے جلے ہوں۔",
    cure: "Streptocycline (Streptomycin + Tetracycline) + Copper Oxychloride 50% WP",
    cureUrdu: "سٹریپٹو سائکلین (اینٹی بائیوٹک) + کاپر آکسی کلورائیڈ",
    dosage: "0.5 g Streptocycline + 2g Copper Oxychloride per Litre of water during bloom.",
    dosageUrdu: "0.5 گرام سٹریپٹو سائکلین + 2 گرام کاپر فی لیٹر پانی۔",
    severity: "High",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTnRXwvtqjBqzp42l3groqzRcuAShSVVsnWUQ&s"
  },
  {
    id: "d9",
    crop: "Apple / سیب",
    name: "Apple Canker",
    nameUrdu: "ایپل کینکر (تنے کا زخم یا کینسر)",
    symptoms: "Sunken, cracked, dying bark around pruning cuts or stem wounds.",
    symptomsUrdu: "چھال پر گہرے زخم، دراڑیں اور لکڑی کا اندر سے سوکھنا۔",
    cure: "Proma Gro (Chaubatia Paste) or Copper Hydroxide Wound Dressing",
    cureUrdu: "پرپروما گرو (چوباٹیا پیسٹ Proma Gro) یا کاپر پیسٹ",
    dosage: "Scrape infected bark clean with knife and paint Proma Gro directly on wood.",
    dosageUrdu: "متاثرہ حصہ صاف کر کے چوباٹیا پیسٹ کا گہرا لیپ کریں۔",
    severity: "High",
    image: "https://www.shutterstock.com/image-photo/apple-tree-canker-on-branch-600nw-2585228479.jpg"
  },
  {
    id: "d10",
    crop: "Apple / سیب",
    name: "Codling Moth",
    nameUrdu: "کوڈلنگ موتھ (سیب کے اندر کی سنڈی)",
    symptoms: "Boreholes in apple skin filled with reddish-brown frass, internal seed damage.",
    symptomsUrdu: "سیب پر سوراخ جس کے گرد بھوری گندگی ہو اور اندر بیج کھایا ہوا ہو۔",
    cure: "Tata Takumi (Flubendiamide 20% WG) or Kozen (Chlorantraniliprole 18.5% SC)",
    cureUrdu: "ٹاٹا تاکومی (Flubendiamide 20%) یا کوزن (Chlorantraniliprole)",
    dosage: "0.5 g Takumi or 0.4ml Kozen per Litre of water after petal fall.",
    dosageUrdu: "0.5 گرام تاکومی یا 0.4 ملی لیٹر کوزن فی لیٹر پانی (پھول گرنے پر)۔",
    severity: "High",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTULXpw0NUvrkFKCw19LtdaxOBu8MUZlMFp-Q&s"
  },
  {
    id: "d11",
    crop: "Walnut / اخروٹ",
    name: "Walnut Blight",
    nameUrdu: "اخروٹ کا بیکٹیریل بلائٹ",
    symptoms: "Black water-soaked spots on leaves, stems, and outer green husk of walnuts.",
    symptomsUrdu: "پتوں اور اخروٹ کے ہری چھال پر سیاہ گیلے نشانات، اخروٹ کا سڑنا۔",
    cure: "Bayer Antracol (Propineb 70% WP) or Copper Hydroxide 77% WP",
    cureUrdu: "بائر اینٹراکول (پروپینیب 70%) یا کاپر ہائیڈرو آکسائیڈ",
    dosage: "2.5 g Antracol per Litre of water during catkin emergence and post-bloom.",
    dosageUrdu: "2.5 گرام اینٹراکول فی لیٹر پانی (نئے پتے اور اخروٹ بنتے وقت)۔",
    severity: "High",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcStpnLbKr6eaHnem_u91UVXJvxalo0ARfrmMQ&s"
  },
  {
    id: "d12",
    crop: "Walnut / اخروٹ",
    name: "Walnut Weevil",
    nameUrdu: "اخروٹ کا کیڑا (ویول)",
    symptoms: "Larvae bore into young walnuts, causing heavy premature fruit drop.",
    symptomsUrdu: "کیڑے کی سنڈیاں اخروٹ کے اندر گھستی ہیں جس سے کچے اخروٹ گر جاتے ہیں۔",
    cure: "Cyclone 505 (Chlorpyrifos 50% + Cypermethrin 5% EC)",
    cureUrdu: "سائکلون 505 (Chlorpyrifos 50% + Cypermethrin 5%)",
    dosage: "2 ml per Litre of water when nuts reach pea-size.",
    dosageUrdu: "2 ملی لیٹر فی لیٹر پانی (جب اخروٹ مٹر کے دانے جتنا ہو)۔",
    severity: "Medium",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQF_LUstlJjIP8cV66gU-M9vQpihIRLkRuIrw&s"
  },
  {
    id: "d13",
    crop: "Walnut / اخروٹ",
    name: "Anthracnose",
    nameUrdu: "اخروٹ کا انتھراکنوز (کالے داغ)",
    symptoms: "Dark brown or black circular spots on leaves and husk, defoliation.",
    symptomsUrdu: "پتوں اور اخروٹ پر گہرے بھورے یا سیاہ دائرے، پتے گرنا۔",
    cure: "Mancozeb 75% WP or Difenoconazole 25% EC",
    cureUrdu: "مینکوزیب 75% ڈبلیو پی یا ڈائفینوکونازول",
    dosage: "2.5 g Mancozeb per Litre of water.",
    dosageUrdu: "2.5 گرام مینکوزیب فی لیٹر پانی۔",
    severity: "Medium",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQgz5lhVOMPg1AIvTsie0xP1ZbgtnQrrKuaqw&s"
  },
  {
    id: "d14",
    crop: "Cherry / چیری",
    name: "Cherry Leaf Spot",
    nameUrdu: "چیری کے پتوں کے سرخ داغ",
    symptoms: "Small purplish-red spots on leaves that turn brown, leaving shot-holes and causing leaf drop.",
    symptomsUrdu: "پتوں پر چھوٹے جامنی یا سرخ دھبے جو بعد میں سوراخ بناتے ہیں اور پتے گرتے ہیں۔",
    cure: "Fargo Super (Captan 50% WP) or Difenoconazole 25% EC",
    cureUrdu: "فارگو سپر (Captan 50%) یا ڈائفینوکونازول",
    dosage: "2 g Captan or 0.5ml Difenoconazole per Litre of water post petal fall.",
    dosageUrdu: "2 گرام کیپٹان یا 0.5 ملی لیٹر ڈائفینوکونازول فی لیٹر پانی۔",
    severity: "High",
    image: "https://upload.wikimedia.org/wikipedia/commons/b/b8/Cherry_Leaf_Spot_2.jpg"
  },
  {
    id: "d15",
    crop: "Cherry / چیری",
    name: "Brown Rot",
    nameUrdu: "چیری کا براؤن راٹ (پھل کا سڑنا)",
    symptoms: "Ripening cherry fruits turn soft, rot rapidly, and get covered with brownish spores.",
    symptomsUrdu: "پکتی ہوئی چیری نرم ہو کر تیزی سے سڑتی ہے اور اس پر بھوری پھپھوندی جمتی ہے۔",
    cure: "Luna Experience (Fluopyram + Tebuconazole)",
    cureUrdu: "لونا ایکسپیرینس (Bayer Luna)",
    dosage: "1 ml per Litre of water sprayed 2-3 weeks before cherry harvest.",
    dosageUrdu: "1 ملی لیٹر فی لیٹر پانی (چیری توڑنے سے 2-3 ہفتے پہلے)۔",
    severity: "High",
    image: "/brown-rot.webp"
  },
  {
    id: "d16",
    crop: "Almond / بادام",
    name: "Shot Hole Disease",
    nameUrdu: "بادام کا شاٹ ہول (پتوں میں سوراخ)",
    symptoms: "Small reddish-brown spots on leaves drop out leaving shotgun-like holes.",
    symptomsUrdu: "پتوں پر لال بھورے نشانات جو جھڑ کر بندوق کے فائر جیسے سوراخ بناتے ہیں۔",
    cure: "Copper Oxychloride 50% WP or Ziram 80% WP",
    cureUrdu: "کاپر آکسی کلورائیڈ 50% یا زیرام 80%",
    dosage: "3 g Copper Oxychloride per Litre during pink bud and dormant stages.",
    dosageUrdu: "3 گرام کاپر فی لیٹر (سردیوں میں اور کلی بنتے وقت)۔",
    severity: "Medium",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQNLQLtbCI4XVuUVl_RJw0gzOciMkGwgEk1aw&s"
  },
  {
    id: "d17",
    crop: "Almond / بادام",
    name: "Peach Leaf Curl",
    nameUrdu: "پتوں کا مڑنا (ریڈ مٹیریل پیپ)",
    symptoms: "Leaves become thick, puckered, curled, and turn bright reddish-yellow.",
    symptomsUrdu: "پتے موٹے، مڑے ہوئے، سلوٹ دار اور سرخ یا پیلے ہو جاتے ہیں۔",
    cure: "Copper Hydroxide 77% WP or Ziram 80% WP",
    cureUrdu: "کاپر ہائیڈرو آکسائیڈ یا زیرام 80%",
    dosage: "3 g per Litre sprayed as a dormant spray before bud swell in spring.",
    dosageUrdu: "3 گرام فی لیٹر (سردیوں کے اختتام پر کپلیاں نکلنے سے پہلے)۔",
    severity: "High",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTC6cXSd3VmSwHigwBn8hjLYK_Cu7B53DEZUQ&s"
  },
  {
    id: "d18",
    crop: "Rice/Paddy / دھان (چاول)",
    name: "Rice Blast",
    nameUrdu: "دھان کا بلاسٹ (چاول کا برباد ہونا)",
    symptoms: "Spindle/diamond-shaped grayish-white lesions with reddish borders on leaves and neck.",
    symptomsUrdu: "پتوں اور گردن پر ہیرے کے سائز کے سفید سرمئی نشان جن کا حاشیہ سرخ ہو۔",
    cure: "Tricyclazole 75% WP (Beam) or Isoprothiolane 40% EC",
    cureUrdu: "ٹرائی سائکلازول 75% (بیم Beam) یا آئسوپروتھیولین",
    dosage: "0.6 g Tricyclazole per Litre of water.",
    dosageUrdu: "0.6 گرام فی لیٹر پانی۔",
    severity: "High",
    image: "https://agritech.tnau.ac.in/crop_protection/images/rice_diseases/blast_1.jpg"
  },
  {
    id: "d19",
    crop: "Rice/Paddy / دھان",
    name: "Brown Spot",
    nameUrdu: "چاول کے بھورے داغ",
    symptoms: "Oval sesame-sized brown spots on leaves and grains causing empty glumes.",
    symptomsUrdu: "پتوں اور دانوں پر تل کے سائز کے بھورے داغ جس سے دانے خالی رہتے ہیں۔",
    cure: "Tropical Tagban (Carbendazim 12% + Mancozeb 63% WP)",
    cureUrdu: "ٹیگ بن (کاربینڈازم + مینکوزیب)",
    dosage: "2 g per Litre of water.",
    dosageUrdu: "2 گرام فی لیٹر پانی۔",
    severity: "Medium",
    image: "https://soybeanresearchinfo.com/wp-content/uploads/2019/03/Brown-spot-Daren-Mueller-35-1300x867.jpg"
  },
  {
    id: "d20",
    crop: "Rice/Paddy / دھان",
    name: "Stem Borer",
    nameUrdu: "دھان کے تنے کا کیڑا (ڈیڈ ہارٹ)",
    symptoms: "Central tiller dries up ('Deadheart'), white chaffy empty earheads ('Whiteheads').",
    symptomsUrdu: "درمیان کا پودا سوکھ جاتا ہے (Deadheart) اور بالیاں سفید و خالی نکلتی ہیں۔",
    cure: "Cartap Hydrochloride 4% GR or Chlorantraniliprole 0.4% GR",
    cureUrdu: "کارٹپ ہائیڈروکلورائیڈ 4% دانے دار",
    dosage: "4 kg granules per acre applied in standing water.",
    dosageUrdu: "4 کلوگرام دانے فی ایکڑ کھڑے پانی میں ڈالیں۔",
    severity: "High",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS1KJH1Y_qQKDCwIFeQ-jUOUUznQSub-fIDIw&s"
  },
  {
    id: "d21",
    crop: "Maize / مکئی",
    name: "Turcicum Leaf Blight",
    nameUrdu: "مکئی کا لمبا جھلساؤ",
    symptoms: "Long, elliptical grayish-green or tan lesions on maize leaves.",
    symptomsUrdu: "مکئی کے پتوں پر لمبے سرمئی یا بھورے نشانات۔",
    cure: "Mancozeb 75% WP or Propiconazole 25% EC",
    cureUrdu: "مینکوزیب 75% یا پروپیکونازول",
    dosage: "2.5 g Mancozeb or 1ml Propiconazole per Litre of water.",
    dosageUrdu: "2.5 گرام مینکوزیب یا 1 ملی لیٹر پروپیکونازول فی لیٹر۔",
    severity: "Medium",
    image: "https://thumbs.dreamstime.com/b/northern-corn-leaf-blight-maize-helminthosporium-turcicum-i-northern-corn-leaf-blight-maize-helminthosporium-turcicum-104797569.jpg"
  },
  {
    id: "d22",
    crop: "Maize / مکئی",
    name: "Fall Armyworm",
    nameUrdu: "فال آرمی ورم (مکئی کی خطرناک سنڈی)",
    symptoms: "Large ragged holes in leaves, heavy frass inside maize leaf whorl.",
    symptomsUrdu: "پتوں پر بڑے سوراخ اور مکئی کی کونپل کے اندر کیڑے کی گندگی۔",
    cure: "Emamectin Benzoate 5% SG or Spinetoram 11.7% SC",
    cureUrdu: "ایمامیکٹن بینزوایٹ 5% ایس جی یا سپینیٹورام",
    dosage: "0.4 g Emamectin Benzoate per Litre of water sprayed directly into whorl.",
    dosageUrdu: "0.4 گرام فی لیٹر پانی (مکئی کے درمیانی حصے میں سپرے کریں)۔",
    severity: "High",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSJ8U0v30vF5C3h20DU2vSXT34Rh_OAlnew_A&s"
  },
  {
    id: "d23",
    crop: "Saffron / زعفران",
    name: "Corm Rot",
    nameUrdu: "زعفران کے گٹھے کا سڑنا (فنگس)",
    symptoms: "Yellowing and stunting of foliage, rotting and black decay of underground corms.",
    symptomsUrdu: "پتوں کا پیلا ہونا اور زمین کے اندر زعفران کے گٹھوں کا گلنا۔",
    cure: "Tropical Tagban (Carbendazim 12% + Mancozeb 63% WP)",
    cureUrdu: "ٹیگ بن (کاربینڈازم + مینکوزیب)",
    dosage: "Dip saffron corms in 2.5g/Litre solution for 15 minutes before planting.",
    dosageUrdu: "کاشت سے 15 منٹ پہلے گٹھوں کو 2.5 گرام فی لیٹر کے محلول میں ڈبوئیں۔",
    severity: "High",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ8AJmDzURXRJziuZVqFVMavcvJqNF8AMsL2A&s"
  },
  {
    id: "d24",
    crop: "Vegetables / سبزیاں",
    name: "Aphids & Thrips",
    nameUrdu: "سبزیوں کا تیلہ اور سوس (کیڑے)",
    symptoms: "Tiny green/black insects sucking sap under leaves, curling leaves, honeydew mold.",
    symptomsUrdu: "پتوں کے نیچے چھوٹے کیڑے جو رس چوستے ہیں جس سے پتے مڑتے ہیں۔",
    cure: "Adunik Starcare (Imidacloprid 17.8% SL) or Thiamethoxam 25% WG",
    cureUrdu: "ایڈونک سٹار کیئر (امیڈاکلوپرڈ) یا تھیامیتھوکسام",
    dosage: "0.5 ml Imidacloprid per Litre of water.",
    dosageUrdu: "0.5 ملی لیٹر فی لیٹر پانی۔",
    severity: "Medium",
    image: "https://images.squarespace-cdn.com/content/v1/502d2cede4b0ab396711e089/57c7fae2-b697-4386-badc-9c959ddc3e32/PredMidge_Uroeucon_aphid_6_2025_PMS+copy+2.jpg"
  },
  {
    id: "d25",
    crop: "Vegetables / سبزیاں",
    name: "Late Blight",
    nameUrdu: "آلو اور ٹماٹر کا پچھیتا جھلساؤ",
    symptoms: "Dark water-soaked lesions on leaves and stems, white mold on underside in humid weather.",
    symptomsUrdu: "پتوں اور تنوں پر گیلے کالی داغ، نمی میں نیچے سفید پھپھوندی بننا۔",
    cure: "Ridomil Gold (Metalaxyl 8% + Mancozeb 64% WP) or Cymoxanil",
    cureUrdu: "ریڈومل گولڈ (Metalaxyl + Mancozeb)",
    dosage: "2.5 g per Litre of water.",
    dosageUrdu: "2.5 گرام فی لیٹر پانی۔",
    severity: "High",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT59UZq2s60UL9MJwEcSN_GPXNlgsSpAsWM9w&s"
  },
  {
    id: "d26",
    crop: "Fruit Trees / پھل دار درخت",
    name: "Phytophthora Root Rot",
    nameUrdu: "جڑوں کا شدید سڑنا (فائٹوف تھورا)",
    symptoms: "Declining canopy, yellowing foliage, dark reddish-brown rotting roots with sour odor.",
    symptomsUrdu: "پتوں کا پیلا ہونا، درخت کا مرجھانا اور جڑوں کا بھورا ہو کر گلنا۔",
    cure: "Ridomil Gold (Metalaxyl + Mancozeb) or Fosetyl-Al 80% WP",
    cureUrdu: "ریڈومل گولڈ یا فوسیٹائل ایلومینیم",
    dosage: "3 g per Litre water. Drench 10-15 Litres around root zone of affected tree.",
    dosageUrdu: "3 گرام فی لیٹر پانی (جڑوں میں 10-15 لیٹر پانی ڈالیں)۔",
    severity: "High",
    image: "https://www.amgrow.com.au/wp-content/uploads/sites/4/2016/08/Pic-2-503x600.jpg"
  },
  {
    id: "d27",
    crop: "Fruit Trees / پھل دار درخت",
    name: "Crown Gall",
    nameUrdu: "کراؤن گال (درخت تنے کا کینسر)",
    symptoms: "Rough, woody tumor-like galls on crown, upper roots, and lower trunk base.",
    symptomsUrdu: "تنے اور جڑ کے جوڑ پر لکڑی نما سخت گلٹیاں یا ٹیومر بننا۔",
    cure: "Surgical Removal + Proma Gro Chaubatia Paste (No chemical spray cure)",
    cureUrdu: "سرجیکل صفائی + پرپروما گرو چوباٹیا پیسٹ",
    dosage: "Chisel away gall down to healthy wood and paint Proma Gro paste on wound.",
    dosageUrdu: "گلٹی کو احتیاط سے کاٹ کر زخم پر چوباٹیا پیسٹ لگائیں۔",
    severity: "Medium",
    image: "https://assets.rhs.org.uk/05836e9f-2692-012f-b4fe-e3f22d127fa9/b1acd355-2e83-4263-b609-2d98564d273c/scn0000013_931579.jpg?w=1200&fit=clip&fm=jpg&auto=format"
  },
  {
    id: "d28",
    crop: "Fruit Trees / پھل دار درخت",
    name: "Armillaria Root Rot",
    nameUrdu: "آرمیلیریا (جڑ سڑنا فنگس)",
    symptoms: "White fan-like fungal sheets beneath bark at soil line, honey-colored mushrooms in autumn.",
    symptomsUrdu: "چھال کے نیچے سفید فنگس کی تہہ اور خریف میں تنے کے پاس کھمبیاں۔",
    cure: "Soil Aeration + IPL Sanjeevni (Trichoderma Viride Bio-Fungicide)",
    cureUrdu: "مٹی کو ہوا دینا + سنجیوانی (ٹرائیکوڈرما وریڈ Bio-Fungicide)",
    dosage: "Expose root collar to sunlight, mix 25g Trichoderma Viride into moist soil.",
    dosageUrdu: "تنے کے نچلے حصے کو ہوا اور دھوپ لگائیں اور 25 گرام ٹرائیکوڈرما مٹی میں ملا دیں۔",
    severity: "High",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9-kFFCxbJd2pHqqbqeIc8mWaAA9MUVkiM4g&s"
  }
];
